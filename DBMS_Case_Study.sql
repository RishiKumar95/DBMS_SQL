...
select ib.bid_team, it.team_id, it.team_name, count(ib.bidder_id) as 'Bidders'
from ipl_bidding_details as ib inner join ipl_team as it
on ib.bid_team = it.team_id
group by ib.bid_team
order by count(bidder_id) asc; 
...
on ib.bid_team = it.team_id
group by ib.bid_team
order by count(bidder_id) desc limit 1;
...
on ib.bid_team = it.team_id
group by ib.bid_team
order by count(bidder_id) asc limit 3;
...
select team_id, sum(matches_played) as 'Total played', sum(matches_won) as 'Total won',
 sum(matches_lost) as 'Total lost'  from ipl_team_standings group by team_id;

select ib.bid_team, it.team_id, it.team_name, count(ib.bidder_id) as 'Bidders'
from ipl_bidding_details as ib inner join ipl_team as it
on ib.bid_team = it.team_id
group by ib.bid_team
order by count(bidder_id); 
...
create view bteam1 as
select team_id, sum(matches_played) as 'Total_played', sum(matches_won) as 'Total_won',
 sum(matches_lost) as 'Total_lost'  from ipl_team_standings group by team_id;
 
 select * from bid;
 select * from bteam1;
 
 select b.team_id, b.team_name,b.bidders, bt.total_played, bt.total_won, bt.total_lost
 from bid as b inner join bteam1 as bt
 on b.team_id = bt.team_id 
 group by b.team_id
 order by bt.total_won desc limit 1;
...
((s2.total_points - s1.total_points)/s1.total_points)*100 as '% diff'
from ipl17 as s1 inner join ipl18 as s2
on s1.team_id = s2.team_id
inner join ipl_team as it
on s1.team_id = it.team_id
group by s1.team_id
order by ((s2.total_points - s1.total_points)/s1.total_points)*100 desc limit 1;
